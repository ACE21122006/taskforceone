...
export interface PayoutMethod {
  id: string;
  user_id: string;
  method: "mpesa" | "airtel_money" | "mixx_by_yas" | "halo_pesa";
  phone_number: string;
  created_at: string;
}
...
export interface OfflineAction {
  id: string;
  type: "submit_proof" | "request_withdrawal" | "send_message";
  payload: any;
  timestamp: string;
}
...
interface AppState {
  ...
  requestWithdrawal: (method: "mpesa" | "airtel_money" | "mixx_by_yas" | "halo_pesa", phone: string, amount: number) => Promise<{ success: boolean; message: string }>;
  ...
  persist(
    (set, get) => ({
      ...
      syncOfflineActions: async () => {
        const { pendingOfflineActions, user, isOnline } = get();
        if (pendingOfflineActions.length === 0 || !user || !isOnline) return;
        
        console.log(`Syncing ${pendingOfflineActions.length} pending offline actions...`);
        const remainingActions: OfflineAction[] = [];
        
        for (const action of pendingOfflineActions) {
          try {
            if (action.type === "submit_proof") {
              const { taskId, coins, notes, screenshot, video } = action.payload;
            ...
            } else if (action.type === "request_withdrawal") {
              const { method, phone, amount } = action.payload;
              ...
              const { messageText, screenshotUrl, coinsSent, userId, senderRole } = action.payload;
        ...
        set({ pendingOfflineActions: remainingActions });
      ...
      initializeData: async () => {
        ...
        try {
          ...
          if (fetchedTasks.length === 0 && initialMockTasks.length > 0) {
            console.log("Seeding initial tasks to Supabase...");
            for (const task of initialMockTasks) {
              const { error: seedErr } = await supabase.from("tasks").insert({
                id: task.id,
                title: task.title,
                reward_tzs: task.reward_tzs,
            ...
            if (reTasksErr) throw reTasksErr;
            fetchedTasks = reTasks || [];
          }
          ...
          const { data: payoutMethods, error: payErr } = await supabase.from("payout_methods").select("*");
          if (payErr) throw payErr;
          
          // Fetch notifications
          const { data: notifications, error: notErr } = await supabase.from("notifications").select("*");
          ...
          // If logged in, update current profile from Supabase list
          const currentUser = get().user;
          let updatedCurrentProfile = get().profile;
          if (currentUser && profiles) {
            updatedCurrentProfile = profiles.find(p => p.id === currentUser.id) || updatedCurrentProfile;
          }
      ...
      submitTaskProof: async (taskId, coins, notes, screenshot, video) => {
        const { isMockMode, user, tasks, submissions, initializeData, isOnline } = get();
        if (!user) return;
        ...
        if (isMockMode) {
          ...
          if (!isOnline) {
            const offlineId = "sub-offline-" + Math.random().toString(36).substring(2, 9);
            const newSubmission: Submission = {
              id: offlineId,
              task_id: taskId,
              user_id: user.id,
            ...
            set({
              submissions: [newSubmission, ...submissions],
              pendingOfflineActions: [
                ...get().pendingOfflineActions,
                {
                  id: offlineId,
                  type: "submit_proof",
                  payload: { taskId, coins, notes, screenshot, video },
      ...
      requestWithdrawal: async (method, phone, amount) => {
        ...
        if (isMockMode) {
          ...
          if (!isOnline) {
            const offlineId = "tx-offline-" + Math.random().toString(36).substring(2, 9);
            const newTx: Transaction = {
              id: offlineId,
              user_id: user.id,
              amount_tzs: -amount,
              type: "withdrawal",
              description: `${method.toUpperCase().replace("_", " ")} withdrawal request to ${phone} (Offline)`,
              status: "pending",
              created_at: new Date().toISOString(),
              username: profile.username
            };
            ...
            set({
              ...
              pendingOfflineActions: [
                ...get().pendingOfflineActions,
                {
                  ...
                  payload: { method, phone, amount },
                  timestamp: new Date().toISOString()
                }
              ]
            });
            return { success: true, message: "Offline mode: Request saved locally and will sync when online." };
          }
      ...
      sendMessage: async (messageText, screenshotUrl, coinsSent, targetUserId) => {
        const { isMockMode, user, profile, messages, initializeData, isOnline } = get();
        if (!user) return;
        ...
        if (isMockMode) {
          ...
          if (!isOnline) {
            const offlineId = "msg-offline-" + Math.random().toString(36).substring(2, 9);
            const newMsg: Message = {
              id: offlineId,
              user_id: userId,
              sender_role: senderRole,
            ...
            set({
              ...
              pendingOfflineActions: [
                ...get().pendingOfflineActions,
                {
                  id: offlineId,
                  type: "send_message",
                  payload: { messageText, screenshotUrl, coinsSent, userId, senderRole },
                  timestamp: new Date().toISOString()
                }
              ]
            });
...
if (typeof window !== "undefined") {
  window.addEventListener("online", () => {
    ...
    useAppStore.getState().syncOfflineActions();
  });
